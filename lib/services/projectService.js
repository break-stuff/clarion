...
class ProjectDataService {
    constructor(_newProductInfo) {
        this._newProductInfo = _newProductInfo;
        this._fileService = new fileService_1.FileService();
        this._projectTypeData = this.getProjectTypeData();
    }
    getProjectData(projectName) {
        if (this._newProductInfo.pipeline !== newProject_1.newProject.options.pipeline.parcel)
            this.createBundleConfigurationFile(projectName);
        if (this._newProductInfo.pipeline !== newProject_1.newProject.options.pipeline.grunt)
    ...
    getConfigFileContents() {
        ...
        if (this._newProductInfo.pipeline === 'Grunt')
            contents.replace(/grunt-less/g, 'grunt-contrib-less');
        return contents;
    }
    getProjectDependencies() {
        let dependencies = this._projectTypeData.devDependencies;
        dependencies = dependencies.concat(this._newProductInfo.styleFormat === newProject_1.newProject.options.styleFormat.less ? this._projectTypeData.lessDependencies : this._projectTypeData.sassDependencies);
        return dependencies;
    }
    getProjectTypeData() {
        switch (true) {
            case this._newProductInfo.pipeline === newProject_1.newProject.options.pipeline.grunt:
                return projectData_1.projectData.grunt;
            case this._newProductInfo.pipeline === newProject_1.newProject.options.pipeline.gulp:
                return projectData_1.projectData.gulp;
            case this._newProductInfo.pipeline === newProject_1.newProject.options.pipeline.parcel:
