...
class Add {
    ...
    addNewDirectory(folderName) {
        ...
        this._fileService.addFileToManifest(`@import './${folderName}/index${extension}';`, `${rootDirectory}/styles${extension}`, true);
    }
    ...
    processNewFile(pathToDirectory, fileName) {
        ...
        if (!this._fileService.fileExists(`${pathToDirectory}/${newFile}`)) {
            ...
            let importStatement = this._config.importAbstracts == "true" &&
            ...
            if (this._config.addToManifest == "true")
                this._fileService.addFileToManifest(newFile, manifestFile, false);
        }
        else {
